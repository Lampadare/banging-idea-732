"""FEM bovine vagosympathetic trunk — fully optimized.

Fixes applied:
  1. BoomerAMG preconditioner (replaces ILU — O(N) memory, no fill-in)
  2. P1 elements (replaces P2 — 4x fewer DOFs)
  3. Mesh coarsening (CharacteristicLengthFactor + Min + shorter nerve)
  4. Footprint caching (1 FEM solve for all amplitudes)
  5. 28-core NEURON parallelism
"""
...
if __name__ == "__main__":
    ...
    log(f"NMOD={nrv.backend.parameters.get_nmod_ncore()}, "
        f"GMSH={nrv.backend.parameters.get_gmsh_ncore()}, "
    ...
    log(f"Geometry: {n_fascicles} fascicles (mixed vagal+sympathetic)")

    # --- Build nerve (shorter: 5mm instead of 10mm) ---
    nerve_d = geom["nerve"]["diameter_um"]
    nerve_l = 5000   # FIX 3: halved from 10000
    outer_d = 10     # mm
    ...
    for i, fg in enumerate(fascicles_geo):
        ...
        diams, types, labels = build_mixed_population(N_AX_PER_FASC, SYMPATHETIC_FRACTION, seed=i)
    ...
    # FIX 2: P1 elements instead of P2
    fem_stim.model.elem = ("Lagrange", 1)
    log("Applied FIX 2: P1 elements (4x fewer DOFs)")
    ...
    # FIX 3: Mesh coarsening — intercept gmsh AFTER mesh is built
    ...
    def patched_compute_mesh(self_mesh):
        ...
        log("  Applying FIX 3: gmsh coarsening")
    ...
    # FIX 4: BoomerAMG — monkey-patch setup_simulations to inject PETSc opts
    ...
    def patched_setup_simulations(self_model):
        ...
        # Now self_model.sim exists — inject BoomerAMG
        log("  Applying FIX 4: BoomerAMG preconditioner (after setup_simulations)")
        ...
        log("  BoomerAMG configured successfully")
    ...
    try:
        ...
        fem_time = time.time() - t0_fem
        log(f"FEM FAILED after {fem_time:.1f}s: {e}")
        import traceback
        traceback.print_exc()
        raise

    # Save footprints
    log("Saving nerve state + footprints...")
    nerve.save("bovine_nerve_optimized.json", extracel_context=True)
    log("Saved: bovine_nerve_optimized.json")
    ...
    all_results = {
        "geometry": {
            "source": geom["source"], "nerve_diameter_um": nerve_d,
            "n_fascicles": n_fascicles, "n_axons_per_fascicle": N_AX_PER_FASC,
            "sympathetic_fraction": SYMPATHETIC_FRACTION,
            "total_fibers": total_fibers, "total_vagal": total_vagal,
            "total_sympathetic": total_symp, "fem_solve_time_s": round(fem_time, 1),
            "optimizations": "P1+BoomerAMG+meshCoarsen+footprintCache+28core",
        },
    ...
    # Save
    with open("bovine_optimized_results.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    with open("bovine_optimized_results.pkl", "wb") as f:
    ...
    log("Saved: bovine_optimized_results.json + .pkl")
